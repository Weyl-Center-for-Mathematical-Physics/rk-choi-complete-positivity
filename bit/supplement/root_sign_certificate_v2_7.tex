...
\hypersetup{pdftitle={Supplemental Material: Certified complete-positivity stability regions and adaptive guards for Runge--Kutta propagation of phase-covariant qubits},pdfauthor={G. Blake Pierpoint; Olivier Bernard; Yichen Liu},pdfsubject={Exact boundary polynomials, Sturm-certified root isolation, sign charts, and no-omission certificate},pdfkeywords={complete positivity, Runge--Kutta methods, Choi matrix, root isolation, sign charts}}
\setlength{\parindent}{0pt}
\setlength{\parskip}{5pt}
\setlength{\emergencystretch}{3em}
\title{Supplemental Material for\\[3pt]\large ``Certified complete-positivity stability regions and adaptive guards for Runge--Kutta propagation of phase-covariant qubits''}
\author{G. Blake Pierpoint$^{1,*}$, Olivier Bernard$^{3,2}$, and Yichen Liu$^{4,2}$\\[6pt]
\small $^1$Old Dominion University, 5115 Hampton Boulevard,\\
\small Norfolk, Virginia 23529, USA\\
\small $^2$Weyl Center for Mathematical Physics, Washington, DC, USA\\
\small $^3$UFR PhITEM, Universit\'e Grenoble Alpes, Saint-Martin-d'H\`eres, France\\
\small $^4$Department of Computer Science and Engineering, Shanghai Jiao Tong University, Shanghai, China\\[3pt]
\small $^*$pierpogb@odu.edu}
\date{August 2026}
\begin{document}
\maketitle
\section*{Scope of this certificate}
This document reconstructs every pure-amplitude-damping CPTP-admissible set reported in the article from exact rational Butcher data. It records the boundary numerators for $a=0$, $a=1$, and $M_R=0$; the full- and half-step stage-resolvent factors; rational isolating intervals for every nonnegative real root; and the sign on every intervening interval. Root counts use Sturm sequences, and interval signs use exact rational sample points. Finite decimal bracket endpoints below denote exact rational numbers. A direct one-step map is CPTP exactly when the RK stage equations are defined and $a(x)\leq1$ and $M_R(x,0)\geq0$; the latter already implies $a(x)\geq0$.
The companion software archive contains the machine-readable JSON certificate, exact sample points and multiplicities, source code, regression tests, and all plotting data. No reported endpoint is inferred from visual sampling; every endpoint is produced by or checked against exact real-root isolation.
\section{Summary}
\begin{center}
