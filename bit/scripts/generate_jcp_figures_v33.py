...
DATA = ROOT / "results" / "jcp_figures_v33"
VERIFY = ROOT / "results" / "v27" / "v27_verification.json"
VERIFY31 = ROOT / "results" / "v33" / "v33_verification.json"
FIG.mkdir(parents=True, exist_ok=True)
DATA.mkdir(parents=True, exist_ok=True)
V = json.loads(VERIFY.read_text(encoding="utf-8"))
V31 = json.loads(VERIFY31.read_text(encoding="utf-8"))

FIXED_DATE = dt.datetime(2026, 8, 14, tzinfo=dt.timezone.utc)
PDF_META = {
    "Title": "Candidate-map complete-positivity figures, v3.3",
    "Author": "G. Blake Pierpoint, Olivier Bernard, Yichen Liu",
    "Creator": "generate_jcp_figures_v33.py",
    "CreationDate": FIXED_DATE,
    "ModDate": FIXED_DATE,
}
PNG_META = {"Software": "generate_jcp_figures_v33.py", "Creation Time": "2026-08-15T00:00:00Z"}
...
def fig5_robustness_noncommuting():
    ...
    ax.set(xlabel=r"transverse coupling $\Omega_x/\Gamma$", ylabel=r"certified step boundary $x$", title=r"(b) Local persistence, $0\leq\Omega_x/\Gamma\leq0.2$")
...
def fig4_tolerance_sweep():
    ...
    fig, axes = plt.subplots(1, 2, figsize=(11.9, 4.5))
    ...
    if strang_rows:
        ...
        ax.loglog([work], [max(err, 1e-18)], "D", ms=6, label="Strang (commuting reference)")
    ...
    ax.grid(which="both", alpha=0.18); ax.legend(frameon=False, fontsize=8)
    ax.text(0.03, 0.04, "Proxy excludes certification, root isolation, and provenance costs.",
            transform=ax.transAxes, fontsize=7.1, va="bottom")
    ...
    ax.bar(pos, direct, label="direct")
    ax.bar(pos, projected, bottom=direct, label="projected")
    ax.bar(pos, rotating, bottom=direct+projected, label="rotating fallback")
    ax.bar(pos, exact, bottom=direct+projected+rotating, label="exact fallback")
    for i, row in enumerate(scenario_rows):
        ax.text(i, direct[i]+projected[i]+rotating[i]+exact[i]+0.12,
                f"searches={int(row['component_searches'])}\nerr={row['global_normalized_choi_error']:.1e}",
                ha="center", va="bottom", fontsize=7)
    ax.set_xticks(pos); ax.set_xticklabels(labels, fontsize=8)
    ax.set(ylabel="accepted candidate maps", title="(b) Candidate-aware actions across distinct regimes")
    ax.set_ylim(0, max((direct+projected+rotating+exact).max()+2.2, 7))
    ax.grid(axis="y", alpha=0.18); ax.legend(frameon=False, fontsize=7.5, ncol=2)
    ...
    with (DATA / "figure4_action_composition.csv").open("w", newline="", encoding="utf-8") as f:
...
def graphical_abstract():
    ...
    ax3.text(.02,.08,"Exact endpoint membership, executed-float semantics, and\nreproducible certificates support reported decisions.",fontsize=11.5)
...
if __name__ == "__main__":
    ...
    graphical_abstract()
    plt.close("all")
    print("Generated v3.3 JCP figures, graphical abstract, and source data.", flush=True)
