...
if int(scenario_rows[2]["component_searches"]) < 1:
    raise RuntimeError("near-saddle case did not exercise component search")
...
if int(projection_row["component_searches"]) < 1:
    raise RuntimeError("adversarial projection test failed to exercise component search")
...
with (OUT / "adaptive_benchmark_v32.csv").open("w", newline="", encoding="utf-8") as handle:
    writer = csv.DictWriter(handle, fieldnames=benchmark_fields)
    writer.writeheader(); writer.writerows(benchmark)
with (OUT / "noncommuting_method_comparison_v32.csv").open("w", newline="", encoding="utf-8") as handle:
...
macro_lines = [
    ...
    rf"\providecommand{{\BenchmarkBufferedSearches}}{{{int(buffered['component_searches'])}}}",
    ...
    rf"\providecommand{{\BenchmarkNearSaddleSearches}}{{{int(near['component_searches'])}}}",
]
(OUT / "benchmark_macros_v32.tex").write_text("\n".join(macro_lines) + "\n", encoding="utf-8")

# Compact main-text table: one representative boundary row plus useful direct/project cases.
...
boundary_repr = [
    next(r for r in sweep_rows if r["policy"] == p and abs(float(r["tolerance"])-1e-3)<1e-15)
    for p in ("error_only", "guard_rotating_fallback", "rotating_frame", "strang")
]
table_rows = boundary_repr + scenario_rows
lines = [
    "% Generated mechanically from results/v32/adaptive_benchmark_v32.csv. Do not edit by hand.",
    r"\begin{table*}[t]", r"\centering", r"\caption{Adaptive benchmark matrix. The boundary row demonstrates safe fallback; buffered and near-saddle rows exercise direct retention, component search, and projection. The reported work counts exclude certification and root-isolation overhead.}", r"\label{tab:adaptive_matrix}",
    r"\resizebox{0.99\textwidth}{!}{%", r"\begin{tabular}{llrrrrrr}", r"\toprule",
    r"Case & Policy & Choi error & Min. eig. & Direct & Projected & Searches & RK stages\\",
    r"\midrule",
]
case_names = {
...
for row in table_rows:
    lines.append(
        ...
        rf"{int(row['component_searches'])} & {int(row['rhs_stage_evaluations'])}\\"
    )
lines.extend([r"\bottomrule", r"\end{tabular}%", r"}", r"\end{table*}"])
(OUT / "adaptive_benchmark_table_v32.tex").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
payload = {
    ...
    "benchmark_macro_source": "results/v32/benchmark_macros_v32.tex",
}
(OUT / "v32_verification.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
md = [
    "# v3.2 external-review closeout verification", "",
    ...
    f"- near-saddle useful case: projected={near['projected_accepts']}, searches={near['component_searches']}, error={near['global_normalized_choi_error']}",
...
(OUT / "v32_verification.md").write_text("\n".join(md)+"\n", encoding="utf-8")
