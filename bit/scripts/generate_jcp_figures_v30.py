...
DATA = ROOT / "results" / "jcp_figures_v30"
VERIFY = ROOT / "results" / "v27" / "v27_verification.json"
VERIFY29 = ROOT / "results" / "v30" / "v30_verification.json"
FIG.mkdir(parents=True, exist_ok=True)
DATA.mkdir(parents=True, exist_ok=True)
V = json.loads(VERIFY.read_text(encoding="utf-8"))
V30 = json.loads(VERIFY29.read_text(encoding="utf-8"))

FIXED_DATE = dt.datetime(2026, 8, 10, tzinfo=dt.timezone.utc)
PDF_META = {
    "Title": "Candidate-map complete-positivity figures, v3.0",
    "Author": "G. Blake Pierpoint, Olivier Bernard, Yichen Liu",
    "Creator": "generate_jcp_figures_v30.py",
    "CreationDate": FIXED_DATE,
    "ModDate": FIXED_DATE,
}
PNG_META = {"Software": "generate_jcp_figures_v30.py", "Creation Time": "2026-08-10T00:00:00Z"}
...
def fig2_candidate_maps():
    direct = V30["candidate_regions_varpi_2"]["full"][0]
    fine = V30["candidate_regions_varpi_2"]["two_half"][0]
...
def fig4_tolerance_sweep():
    rows = [r for r in V30["benchmark_rows"] if r["scenario"] == "boundary_tolerance_sweep"]
    fig, axes = plt.subplots(1, 2, figsize=(11.8, 4.4))
    ax = axes[0]
...
def graphical_abstract():
    ...
    direct=V30["candidate_regions_varpi_2"]["full"][0]; fine=V30["candidate_regions_varpi_2"]["two_half"][0]
...
if __name__ == "__main__":
    ...
    graphical_abstract()
    plt.close("all")
    print("Generated v3.0 JCP figures, graphical abstract, and source data.", flush=True)
