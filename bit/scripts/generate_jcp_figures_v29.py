...
# Isolate Matplotlib cache/lock files so repeated clean-room runs cannot block
# on a stale shared font-cache lock in container or CI environments.
os.environ.setdefault("MPLCONFIGDIR", f"/tmp/jcp_v29_mplconfig_{os.getpid()}")
...
DATA = ROOT / "results" / "jcp_figures_v29"
VERIFY = ROOT / "results" / "v27" / "v27_verification.json"
VERIFY29 = ROOT / "results" / "v29" / "v29_verification.json"
FIG.mkdir(parents=True, exist_ok=True)
DATA.mkdir(parents=True, exist_ok=True)
V = json.loads(VERIFY.read_text(encoding="utf-8"))
V29 = json.loads(VERIFY29.read_text(encoding="utf-8"))

FIXED_DATE = dt.datetime(2026, 8, 10, tzinfo=dt.timezone.utc)
PDF_META = {
    "Title": "Candidate-map complete-positivity figures, v2.9",
    "Author": "G. Blake Pierpoint, Olivier Bernard, Yichen Liu",
    "Creator": "generate_jcp_figures_v29.py",
    "CreationDate": FIXED_DATE,
    "ModDate": FIXED_DATE,
}
PNG_META = {"Software": "generate_jcp_figures_v29.py", "Creation Time": "2026-08-10T00:00:00Z"}
...
def fig1_topology_conditioning():
    ...
    inset.text(vm + 0.0015, 2.63, r"$\varpi_-=0.827$", fontsize=6.5, rotation=90, va="top")
    inset.text(vu + 0.0015, 2.63, r"$\sqrt{3}/2=0.866$", fontsize=6.5, rotation=90, va="top")
...
def fig2_candidate_maps():
    direct = V29["candidate_regions_varpi_2"]["full"][0]
    fine = V29["candidate_regions_varpi_2"]["two_half"][0]
...
def fig4_tolerance_sweep():
    rows = [r for r in V29["benchmark_rows"] if r["scenario"] == "boundary_tolerance_sweep"]
    fig, axes = plt.subplots(1, 2, figsize=(11.8, 4.4))
    ax = axes[0]
    for policy in ("error_only", "candidate_guard", "rotating_frame"):
        ...
        ax.loglog(work, err, "o-", label=policy.replace("_", " "))
    ...
    ax.set(xlabel="work proxy (RK stages + 3 per exponential action)", ylabel="global normalized-Choi trace error", title="(a) Accuracy versus reported work across requested tolerances")
    ax.text(0.03, 0.045, r"Markers follow tolerances $10^{-2},3\times10^{-3},10^{-3},3\times10^{-4}$." + "\n" + "Proxy: 1 per RK stage, 3 per exponential action; no universal timing rank is claimed.", transform=ax.transAxes, fontsize=6.9)
    ax.grid(which="both", alpha=0.18); ax.legend(frameon=False, fontsize=8)

    ax = axes[1]
    policies = ("error_only", "candidate_guard", "rotating_frame", "strang")
    tol = 1e-3
    ...
    ax.set_yscale("log"); ax.set_xticks(pos); ax.set_xticklabels([p.replace("_", "\n") for p in policies], fontsize=8)
    ...
    ax.text(0.02, 0.05, "Strang is exact for this commuting split; it is a structural reference, not a timing claim.", transform=ax.transAxes, ha="left", va="bottom", fontsize=6.9)
...
def graphical_abstract():
    ...
    direct=V29["candidate_regions_varpi_2"]["full"][0]; fine=V29["candidate_regions_varpi_2"]["two_half"][0]
...
if __name__ == "__main__":
    ...
    graphical_abstract()
    print("Generated v2.9 JCP figures, graphical abstract, and source data.")
