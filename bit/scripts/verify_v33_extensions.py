...
with (OUT / "adaptive_benchmark_v33.csv").open("w", newline="", encoding="utf-8") as handle:
    writer = csv.DictWriter(handle, fieldnames=benchmark_fields)
    writer.writeheader(); writer.writerows(benchmark)
with (OUT / "noncommuting_method_comparison_v33.csv").open("w", newline="", encoding="utf-8") as handle:
...
macro_lines = [
    "% Generated mechanically from results/v33/adaptive_benchmark_v33.csv. Do not edit by hand.",
...
(OUT / "benchmark_macros_v33.tex").write_text("\n".join(macro_lines) + "\n", encoding="utf-8")
...
(OUT / "adaptive_benchmark_table_v33.tex").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
payload = {
    ...
    "benchmark_macro_source": "results/v33/benchmark_macros_v33.tex",
}
(OUT / "v33_verification.json").write_text(json.dumps(payload, indent=2), encoding="utf-8")
md = [
...
(OUT / "v33_verification.md").write_text("\n".join(md)+"\n", encoding="utf-8")
