...
DATA = ROOT / "results" / "jcp_figures_v28"
VERIFY = ROOT / "results" / "v27" / "v27_verification.json"
VERIFY28 = ROOT / "results" / "v28" / "v28_verification.json"
FIG.mkdir(parents=True, exist_ok=True)
DATA.mkdir(parents=True, exist_ok=True)
V = json.loads(VERIFY.read_text(encoding="utf-8"))
V28 = json.loads(VERIFY28.read_text(encoding="utf-8"))

FIXED_DATE = dt.datetime(2026, 8, 10, tzinfo=dt.timezone.utc)
PDF_META = {
    "Title": "Candidate-map complete-positivity figures, v2.8",
    "Author": "G. Blake Pierpoint, Olivier Bernard, Yichen Liu",
    "Creator": "generate_jcp_figures_v28.py",
    "CreationDate": FIXED_DATE,
    "ModDate": FIXED_DATE,
}
PNG_META = {"Software": "generate_jcp_figures_v28.py", "Creation Time": "2026-08-10T00:00:00Z"}
...
def fig2_candidate_maps():
    direct = V28["direct_components_varpi_2"][0]
    fine = V28["two_half_components_varpi_2"][0]
    ...
    ax.set(xlim=(0, 2.75), ylim=(-0.65, 1.65), yticks=[], xlabel=r"total step $H=\Gamma h$ at $\varpi=2$", title="(a) Coarse and fine candidates have non-nested CPTP sets")
...
def fig4_tolerance_sweep():
    ...
    for policy in ("error_only", "candidate_guard", "rotating_frame"):
        ...
        for xx, yy, row in zip(work, err, data):
            ax.annotate(f"{row['tolerance']:.0e}", (xx,yy), xytext=(3,3), textcoords="offset points", fontsize=6.5)
    ax.set(xlabel="reported work units (RK stages + 3 per exponential action)", ylabel="global normalized-Choi trace error", title="(a) Accuracy-work tolerance sweep")
    ax.grid(which="both", alpha=0.18); ax.legend(frameon=False, fontsize=8)
    ...
    width=.36
    ax.bar(pos-width/2, errors, width, label="global trace error")
    ...
    ax.text(2.55, 2e-16, "Strang is exact for this commuting split;\nit is a structural reference, not a timing claim.", ha="center", fontsize=7)
...
def graphical_abstract():
    ...
    direct=V28["direct_components_varpi_2"][0]; fine=V28["two_half_components_varpi_2"][0]
    ...
    ax3.text(.02,.68,"Coarse, fine, and extrapolated RK candidates\nhave distinct CPTP stability regions.",fontsize=13)
...
if __name__ == "__main__":
    ...
    graphical_abstract()
    print("Generated v2.8 JCP figures, graphical abstract, and source data.")
