#!/usr/bin/env python3
"""Current-version third-order adversarial release gate for JCP v3.2.

This gate verifies that the v3.1 hostile-audit defects remain closed in the
current v3.2 source/results: theorem-domain validation, current-result-bound
...
OUT = ROOT / "independent_validation" / "third_order_adversarial_v32_report.json"
...
# 2. Current benchmark and TeX macros must be v3.2-bound and labels honest.
csv_path = ROOT / "results" / "v32" / "adaptive_benchmark_v32.csv"
macro_path = SOURCE / "generated" / "benchmark_macros_v32.tex"
...
if not (csv_path.exists() and macro_path.exists() and section_path.exists() and article_path.exists()):
    findings.append({"id": "MISSING-V32-BENCHMARK-BINDING"})
else:
    ...
    if "benchmark_macros_v32" not in article or "BenchmarkRotatingError" not in macros:
        findings.append({"id": "V32-MACROS-NOT-BOUND"})
