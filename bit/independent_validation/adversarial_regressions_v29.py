#!/usr/bin/env python3
"""Independent adversarial regressions for JCP v2.8.
...
def main() -> int:
    ...
    def rotating_candidate_matrix_identity() -> dict[str, Any]:
        ...
        for h in (0.5, 1.0, 2.0):
            ...
            proper = _rotating_candidate(
                total_x=h, theta=0, kappa=0, varpi=2, substeps=2
            )
            proper_matrix = phase_covariant_superoperator_numeric(proper)
            stored_norm = float(np.linalg.norm(stored_matrix - ev.fine_matrix))
            proper_norm = float(np.linalg.norm(proper_matrix - ev.fine_matrix))
    ...
    def benchmark_projection_coverage() -> dict[str, Any]:
        csv_path = root / "results" / "v28" / "adaptive_benchmark_v28.csv"
        ...
        with csv_path.open(newline="", encoding="utf-8") as handle:
            guarded = [
                row for row in csv.DictReader(handle) if row.get("policy") == "candidate_guard"
            ]
    ...
    def richardson_alpha_static_check() -> dict[str, Any]:
        path = root / "scripts" / "verify_v28_extensions.py"
